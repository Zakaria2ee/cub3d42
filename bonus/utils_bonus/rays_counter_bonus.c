...
/*   Updated: 2022/08/25 16:24:17 by mabenchi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	reach_wall(t_data *data, double angle, double *x, double *y)
{
	...
	*x = data->player_x;
	*y = data->player_y;
	while (!walls_checker31_b(*x, *y, angle, data))
	{
	...
	*x -= 1 * cos(angle * PI / 180);
	*y -= 1 * sin(angle * PI / 180);
	d_ray = walls_checker301_b(*x, *y, angle, data);
	while (!d_ray)
	{
...
static int	get_t_width_dir(t_data *data, double x, double y, int d_ray)
{
	...
	{
		if (d_ray == 's' || d_ray == 'n')
			return ((int)round(y * ((double)data->door.w / 50.0))
				% data->door.w);
		else
			return ((int)round(x * ((double)data->door.w / 50.0))
				% data->door.h);
	}
	if (d_ray == 's')
